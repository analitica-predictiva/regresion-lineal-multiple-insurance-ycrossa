...
def pregunta_03():
    ...
    """print(OneHotEncoder().get_params().keys())
    ...
    X_train2 = m.fit_transform(X_train)
    print("X_train2",X_train2)

    sk_best = SelectKBest(score_func=f_regression)
    x_new = sk_best.fit_transform(X_train2,y_train)
    print("x_new",x_new)

    lin = LinearRegression().fit(x_new,y_train)
    print(lin.score(x_new,y_train))
    exit()"""
    ...
    #['categories', 'drop', 'dtype', 'handle_unknown', 'sparse']
    param_grid = {
